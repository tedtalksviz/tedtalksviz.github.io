...
def get_location(soup, event_name):
    ...
    for row in rows[:5]:
        tds = row.findAll('td')
        if len(tds) == 5:
            name = tds[1].find('strong').find('a').string.replace('&shy', '').replace(';\xad','').replace('\xad','')
...
def scrape(event_name, year):
    city = ''
    country = ''
    url = 'https://www.ted.com/tedx/events?when=past&autocomplete_filter={0}&year={1}'.format(event_name, year)
